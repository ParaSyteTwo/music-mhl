...
export default function DownloadsPage() {
  ...
  return (
    ...
    >
      ...
      {failed.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xs font-mono uppercase tracking-widest text-[#9E9E98] mb-3">{t('failed')}</h2>
          <div className="space-y-1.5">
            {failed.map((dl) => (
              <div
                key={dl.id}
                className="flex items-center gap-2.5 sm:gap-3 px-3 py-2.5 rounded-xl bg-[rgba(244,63,94,0.04)] border border-[rgba(244,63,94,0.15)] hover:bg-[rgba(244,63,94,0.07)] transition-colors"
              >
                <XCircle className="w-4 h-4 text-red-400 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] sm:text-sm font-semibold truncate text-[#F5F5F0]">{dl.track.title}</p>
                  <p className="text-[11px] sm:text-xs text-red-400/90 line-clamp-2 break-words mt-0.5">{dl.error || t('unknownError')}</p>
                </div>
                <button
                  onClick={() => startDownload(dl.track)}
                  className="text-xs font-semibold text-[#C8F04B] hover:underline px-2.5 py-1.5 min-h-[44px] flex items-center"
                >
                  {t('retry')}
                </button>
                <button
                  onClick={() => removeDownload(dl.id)}
                  className="p-2 -mr-1 text-[#9E9E98] hover:text-red-400 active:text-red-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg"
                >
      ...
      {completed.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xs font-mono uppercase tracking-widest text-[#9E9E98] mb-3">{t('completed')}</h2>
          <div className="space-y-1.5">
            {(showAllCompleted ? completed : completed.slice(0, 2)).map((dl) => (
              <div
                key={dl.id}
                className="flex items-center gap-2.5 sm:gap-3 px-3 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-white/[0.1] active:bg-white/[0.06] transition-colors group"
              >
                <CheckCircle className="w-4 h-4 text-[#C8F04B] shrink-0" />
                <div className="w-9 h-9 sm:w-8 sm:h-8 rounded-lg overflow-hidden shrink-0 border border-white/10">
                  {dl.track.cover ? (
                    ...
                    <div className="w-full h-full bg-[rgba(200,240,75,0.1)] flex items-center justify-center">
                      <Music className="w-3 h-3 text-[#9E9E98]" />
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] sm:text-sm font-semibold truncate text-[#F5F5F0]">{dl.track.title}</p>
                  <p className="text-[11px] sm:text-xs text-[#9E9E98] truncate">{dl.track.artist} · {dl.track.album}</p>
                </div>
