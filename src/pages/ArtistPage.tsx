...
import { fetchArtistDetail } from '../lib/api/musicApi';
import { Skeleton } from '../components/ui/skeleton';

type ArtistTab = 'popular' | 'discography' | 'similar';

interface ArtistData {
  info: { id: number; name: string; picture: string; fans: number };
  topTracks: any[];
  albums: any[];
  related: any[];
}
...
export function ArtistPage() {
  ...
  const { playTrack, playQueue } = useMusicStore();
    ...
    return (
      <div className="min-h-screen bg-[#080808] pt-6 pb-32 flex items-center justify-center">
        <div className="text-center">
          ...
          <a href="/search" className="text-[#C8F04B] hover:underline">Back to Search</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#080808] pb-32">
      <div className="h-72 relative flex items-end p-8"
        style={{ backgroundImage: `url(${data.info.picture})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative space-y-4">
          ...
          <p className="text-lg text-[#999999]">{formatFans(data.info.fans)} fans</p>
          <div className="flex gap-3">
            <button onClick={() => data.topTracks.length > 0 && playQueue(data.topTracks, 0)}
              ...
              <Play size={16} className="ml-0.5" /> Reproducir todo
            </button>
            <button onClick={() => data.topTracks.length > 0 && playQueue([...data.topTracks].sort(() => Math.random() - 0.5), 0)}
              className="px-6 py-2 border border-[rgba(255,255,255,0.2)] text-[#F5F5F0] rounded-lg hover:bg-[rgba(255,255,255,0.05)] transition-colors">
              Shuffle
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        <div className="flex gap-6 mb-8 border-b border-[rgba(255,255,255,0.06)] pb-4">
          {([['popular', 'Popular'], ['discography', 'Discografía'], ['similar', 'Similares']] as const).map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`font-dm-sans text-sm transition-colors ${tab === id ? 'text-[#C8F04B] border-b-2 border-[#C8F04B]' : 'text-[#666660] hover:text-[#F5F5F0]'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === 'discography' && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {data.albums.map((album) => (
              <button key={album.id} onClick={() => navigate(`/album/${album.deezerId}`)}
